...
from app.shared.exceptions import UserAlreadyExistsException, EmailVerificationPendingException
...
class AuthService:
    ...
    async def register(self, schemas: RegistrationSchema, background_task: BackgroundTasks):
        ...
        if check_user_staged is not None:
            match await self.repo.check_is_staged_email_verified(schemas.email):
                case RegStagedState.IS_VERIFIED:
                    # delete from redis
                    await self.repo.delete_stage_registration(check_user_staged.reg_dt.email)
                    await self.user_repo.create_user(check_user_staged.reg_dt)
        ...
        if check_user_staged is None:
            otp_key = get_token_key(schemas.email, OtpTokenType.VERIFICATION)
            otp = self.security.generate_otp()
            background_task.add_task(self.email_service.send_email_otp, email=schemas.email, otp=otp.otp)  
            
            background_task.add_task(self.repo.store_temp_otp, data=otp, key=otp_key)
    ...
    async def verify_otp_code(self, otp: str, email: str):
        pass
